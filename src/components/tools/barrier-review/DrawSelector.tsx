    ...
    <div className="flex flex-col items-center gap-6 w-full max-w-lg mx-auto">
      <div className="relative w-full bg-neutral-900 rounded-lg border-4 border-amber-600 shadow-[0_0_50px_rgba(217,119,6,0.15)] overflow-hidden">
        ...
        <div className="relative w-full bg-black flex items-center justify-center" style={{ height: `${VISIBLE_ITEMS * ITEM_HEIGHT}px` }}>
          ...
          <div className="absolute left-0 right-0 top-1/2 -translate-y-1/2 z-10 border-y-2 border-amber-400/50 bg-amber-500/10 pointer-events-none shadow-[0_0_30px_rgba(245,158,11,0.2)]" style={{ height: `${ITEM_HEIGHT}px` }}>
            <div className="absolute left-2 top-1/2 -translate-y-1/2 w-0 h-0 border-l-[8px] border-l-amber-500 border-y-[6px] border-y-transparent" />
            <div className="absolute right-2 top-1/2 -translate-y-1/2 w-0 h-0 border-r-[8px] border-r-amber-500 border-y-[6px] border-y-transparent" />
          </div>
              ...
              {stripItems.map((item, index) => {
                ...
                return (
                  <div key={`${item.id}-${index}`} className="w-full flex items-center justify-center px-4 text-center border-b border-neutral-900/50" style={{ height: `${ITEM_HEIGHT}px` }}>
                    <span className={`text-lg md:text-2xl font-bold uppercase tracking-wider transition-all duration-500 ${isWinner ? "text-amber-400 scale-110 drop-shadow-[0_0_10px_rgba(245,158,11,0.8)]" : "text-neutral-600 blur-[0.5px]"}`}>
                      {item.label}
                    </span>
                  </div>
                );
        ...
        <div className="flex bg-neutral-900/80 p-1 rounded-sm border border-neutral-800">
          {SPEED_OPTIONS.map((opt) => (
            <button
              ...
              disabled={isSpinning}
              className={`flex-1 py-3 text-[10px] md:text-xs font-bold uppercase tracking-widest transition-all rounded-sm ${selectedSpeed.id === opt.id ? "bg-amber-500 text-black shadow-[0_0_10px_rgba(245,158,11,0.3)]" : "text-neutral-500 hover:text-neutral-300 hover:bg-neutral-800"} ${isSpinning ? "opacity-50 cursor-not-allowed" : "cursor-pointer"}`}
            >
              {opt.label}
            </button>
          ))}
      ...
      <button
        onClick={spin}
        disabled={isDisabled}
        className={`relative w-full group overflow-hidden px-8 py-5 rounded-sm text-lg font-black uppercase tracking-[0.2em] transition-all duration-200 ${isDisabled ? "bg-neutral-900 text-neutral-600 cursor-not-allowed border border-neutral-800" : "bg-amber-500 hover:bg-amber-400 text-black cursor-pointer shadow-[0_0_30px_rgba(245,158,11,0.4)] border border-amber-300"}`}
      >
