    ...
    <div className="fixed inset-0 z-[60] flex items-center justify-center p-4">
      ...
      <div className="relative bg-neutral-900 border border-neutral-700 w-full max-w-lg max-h-[85vh] flex flex-col rounded-sm shadow-[0_0_50px_rgba(0,0,0,0.8)]">
        ...
        <div className="px-6 py-4 border-b border-neutral-800 bg-neutral-900/50 flex gap-3 z-10">
          <button onClick={() => onToggleAll(false)} className="flex-1 text-xs font-bold uppercase tracking-wider text-black bg-amber-500 hover:bg-amber-400 px-4 py-2 rounded-sm transition-colors">
            Select All
          </button>
          <button onClick={() => onToggleAll(true)} className="flex-1 text-xs font-bold uppercase tracking-wider text-neutral-400 bg-neutral-800 hover:bg-neutral-700 hover:text-white px-4 py-2 rounded-sm transition-colors">
            Deselect All
          </button>
        </div>
            ...
            return (
              ...
              >
                <div className="flex flex-col">
                  <span className={`text-sm font-bold transition-colors ${isSelected ? "text-white" : "text-neutral-500"}`}>{item.label}</span>
                  <span className="text-[10px] text-neutral-600 uppercase tracking-wider">{item.source}</span>
                </div>
                <div className={`w-6 h-6 rounded-sm flex items-center justify-center transition-all duration-200 ${isSelected ? "bg-amber-500 shadow-[0_0_10px_rgba(245,158,11,0.4)]" : "bg-neutral-800 border-2 border-neutral-700 group-hover:border-neutral-500"}`}>
                  {isSelected && (
        ...
        <div className="p-6 border-t border-neutral-800 bg-neutral-900 rounded-b-sm">
          <button onClick={onClose} className="w-full bg-white hover:bg-neutral-200 text-black font-black uppercase tracking-[0.2em] py-4 transition-colors rounded-sm">
