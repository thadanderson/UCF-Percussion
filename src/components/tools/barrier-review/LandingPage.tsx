...
const LandingPage: React.FC<LandingPageProps> = ({ onEnter }) => {
  return (
    <div className="bg-ucf-black text-ucf-white flex flex-col items-center justify-center p-6 relative overflow-hidden min-h-[80vh]">
      ...
      <div className="relative z-10 max-w-5xl w-full flex flex-col items-center text-center space-y-12">
        <div className="space-y-6">
          <div className="inline-block border-b-2 border-ucf-gold pb-2 mb-2">
            <span className="text-ucf-gold text-xs font-bold uppercase tracking-[0.3em]">UCF Percussion Studies</span>
          </div>
          <h1 className="font-black text-ucf-white tracking-tight leading-none">
            <span className="block text-6xl md:text-9xl mb-2">BARRIER</span>
            <span className="block text-2xl md:text-5xl text-ucf-gold tracking-wide">
              REVIEW &amp; DRAWING
            </span>
          </h1>
          <p className="text-neutral-400 text-lg md:text-xl max-w-2xl mx-auto font-light border-l-2 border-ucf-gold/50 pl-6 italic text-left md:text-center md:border-l-0 md:pl-0 mt-6">
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-6 w-full mt-8 text-left">
          {[
            ...
            <div key={idx} className="bg-neutral-900 border border-neutral-800 p-8 rounded-sm hover:border-ucf-gold transition-colors group relative overflow-hidden">
              <div className="absolute top-0 right-0 p-4 opacity-10 group-hover:opacity-20 transition-opacity">
                <span className="text-6xl font-black text-ucf-gold">{item.step}</span>
              </div>
              <h3 className="text-ucf-white font-bold uppercase tracking-wider mb-3 text-lg">{item.title}</h3>
        ...
        <div className="pt-8">
          <button
            onClick={onEnter}
            className="group relative px-12 py-6 bg-ucf-gold hover:bg-ucf-gold/90 text-black font-black uppercase tracking-[0.2em] transition-all hover:shadow-[0_0_50px_rgba(255,201,4,0.5)]"
          >
