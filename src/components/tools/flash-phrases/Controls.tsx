...
const Controls: React.FC<ControlsProps> = ({
  ...
  const inactiveBtn = "bg-gray-800 border border-gray-700 text-gray-400 hover:border-gray-600";
      ...
      <div className={`overflow-hidden transition-all duration-300 ease-in-out ${isOpen ? 'max-h-[300px] opacity-100' : 'max-h-0 opacity-0'}`}>
        <div className="p-4 pt-2">
          <div className="max-w-7xl mx-auto grid grid-cols-2 md:grid-cols-4 gap-3">
            ...
            {/* 1. BACKING TRACK */}
            <div className={`${cardBase} bg-gray-900 border-gray-700 justify-between`}>
              ...
              <div className="flex flex-col gap-2 pt-1">
                <button onClick={toggleBackingTrack}
                  ...
                  <div className={`w-2.5 h-2.5 rounded-full transition-colors ${isBackingTrack ? 'bg-ucf-gold shadow-[0_0_8px_rgba(255,201,4,0.6)]' : 'bg-gray-600'}`} />
                </button>
                {isBackingTrack && (
                  <select value={currentProgressionId} onChange={e => onProgressionSelect(e.target.value)}
                    disabled={progressionLoading}
                    className="w-full bg-gray-800 text-white text-xs font-medium rounded-md border border-gray-600 p-2 focus:ring-1 focus:ring-ucf-gold appearance-none disabled:opacity-50">
            ...
            <div className={`${cardBase} bg-gray-900 border-gray-700 justify-between`}>
              ...
              <div className="flex-1 flex flex-col justify-end pb-0.5">
                <button onClick={toggleShuffle}
                  ...
                  <div className={`w-2.5 h-2.5 rounded-full transition-colors ${isShuffle ? 'bg-ucf-gold shadow-[0_0_8px_rgba(255,201,4,0.6)]' : 'bg-gray-600'}`} />
                </button>
              </div>
            </div>
              ...
              <div className="flex-1 flex flex-col justify-end pb-0.5">
                <div className="flex">
                  <select value={currentPhraseIndex} onChange={(e) => onPhraseSelect(parseInt(e.target.value))}
                    className="flex-1 bg-gray-800 text-white text-xs font-medium rounded-l-md border border-gray-600 focus:ring-1 focus:ring-ucf-gold block w-full p-2 appearance-none truncate">
                    {currentSubCategory.phrases.map((p, idx) => (
                      <option key={p.id} value={idx}>{idx + 1}. {p.name}</option>
                    ))}
                  </select>
                  <button onClick={onOpenFilter}
                    className="bg-gray-700 hover:bg-gray-600 text-white px-3 border-y border-r border-gray-600 rounded-r-md flex items-center justify-center transition-colors relative"
                    title="Filter Cards">
                    <ListChecks className="w-4 h-4" />
                    {excludedIndices.length > 0 && (
                      <span className="absolute -top-1 -right-1 w-2.5 h-2.5 bg-ucf-gold rounded-full border border-gray-800" />
                    )}
                  </button>
                </div>
              </div>
